...
Twilio::Rails.setup do |config|
  ...
  config.account_sid = "TODO"
  config.auth_token = "TODO"
  ...
  config.default_outgoing_phone_number = "TODO"
  ...
  config.host = "https://tre.ngrok.io"
  ...
  config.phone_trees.register { VoiceTestTree }

  config.include_phone_macros Module.new {
    def last_transcription(response)
      response.phone_call.responses.prompt(:say_something).last.transcription.presence
    end

    def last_transcription_answer(response)
      text = response.phone_call.responses.prompt(:say_something).last.answer.presence
      text.split("\n").map { |s| s.strip.match?(/[.?!]$/) ? s : "#{ s.strip }." }.join(" ")
    end
  }
end
